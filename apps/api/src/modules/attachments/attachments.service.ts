...
import { sanitizePathSegment } from "./upload.constraints";
import {
  ConfirmUploadDto,
  CreateUploadUrlDto,
...
export class AttachmentsService {
  ...
  // Sanitize any single path segment down to a safe charset so no
  // caller-supplied value (entityType/entityId/filename) can escape UPLOAD_DIR
  // via "../" or absolute paths. Reject if nothing survives.
  private safeSegment(raw: string, label: string): string {
    const cleaned = sanitizePathSegment(raw);
    if (!cleaned) {
      throw new BusinessError(
        "INVALID_PATH_SEGMENT",
        `Invalid ${label} for storage path`,
  ...
  private buildRelPath(
    ...
    const safeType = this.safeSegment(relatedType.toLowerCase(), "entityType");
    const safeId = this.safeSegment(relatedId, "entityId");
    const safeName = this.safeSegment(fileName, "fileName");
    const ts = Date.now();
    return `${safeType}/${safeId}/${ts}-${safeName}`;
  }

  private absPath(relPath: string): string {
    // Defence in depth: even for rows read back from the DB, ensure the
    // resolved path stays inside UPLOAD_DIR before touching the filesystem.
    const abs = resolve(this.uploadDir, relPath);
    const root = this.uploadDir.endsWith(sep) ? this.uploadDir : this.uploadDir + sep;
    if (abs !== this.uploadDir && !abs.startsWith(root)) {
      throw new BusinessError(
        "INVALID_STORAGE_PATH",
        "Resolved storage path escapes the upload directory",
        HttpStatus.BAD_REQUEST,
      );
    }
    return abs;
  ...
  async softDelete(id: string) {
    ...
    // File vật lý dùng chung giữa nhiều row (ảnh phiếu mua copy sang máy) —
    // chỉ xóa storage khi không còn row nào khác trỏ tới.
    const shared = await this.repo.anotherRowSharesStorage(a.id, a.driveFileId, a.fileUrl);
    if (shared) {
      this.logger.log(`Attachment ${id}: storage còn row khác dùng — giữ file`);
  ...
  /**
   * Xóa toàn bộ attachment của một entity (dùng khi entity bị xóa hẳn —
   * đơn mua / máy / linh kiện). File vật lý xóa best-effort; gọi SAU khi
   * transaction nghiệp vụ đã commit vì thao tác file không rollback được.
   */
  async deleteAllFor(relatedTypes: string[], relatedId: string): Promise<number> {
    const rows = await this.repo.findByRelated(relatedTypes, relatedId);
    for (const a of rows) {
      await this.dbs.transaction(async (db) => {
        await this.repo.deleteById(a.id, db);
      });
      // Row khác còn dùng chung file (copy ảnh phiếu mua → máy) thì giữ storage.
      const shared = await this.repo.anotherRowSharesStorage(a.id, a.driveFileId, a.fileUrl);
      if (shared) {
        continue;
      }
      if (a.driveFileId) {
        await this.drive
          .deleteFile(a.driveFileId)
          .catch((err: Error) =>
            this.logger.warn(`Failed to remove Drive file ${a.driveFileId}: ${err.message}`),
          );
      } else {
        await unlink(this.absPath(a.fileUrl)).catch((err: Error) =>
          this.logger.warn(`Failed to remove local file ${a.fileUrl}: ${err.message}`),
        );
      }
    }
    return rows.length;
