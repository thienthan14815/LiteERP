...
export const users = sqliteTable("users", {
  ...
  // Auth hardening (migration 0001). Columns absent on DB files created before
  // that migration — the runner adds them at boot.
  failedLoginCount: integer("failed_login_count").notNull().default(0),
  lockedUntil: timestamp("locked_until"),
  mustChangePassword: boolCol("must_change_password").notNull().default(false),
...
// Durable job ledger (migration 0002). Backs the in-process queues so a job
// interrupted by a crash is visible and reconcilable on next boot.
...
export const schema = {
  ...
  refreshTokens, codeCounters, backupRecords, auditLogs, jobs,
