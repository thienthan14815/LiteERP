...
function FinishedPcDetailBody() {
  const searchParams = useSearchParams();
  const id = searchParams.get("id") ?? "";
  const router = useRouter();
  const { data, isLoading, isError } = useFinishedPc(id);
  const updateMut = useUpdateFinishedPc(id);
  const transitionMut = useTransitionFinishedPc(id);
  const scrapMut = useScrapFinishedPc(id);
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            {data.assemblyOrder && (
              <div className="flex justify-between">
                ...
                <Link href={`/assemblies/detail?id=${data.assemblyOrder.id}`} className="underline">
            ...
            {data.sourceMachine && (
              <div className="flex justify-between">
                <span className="text-muted-foreground">Máy gốc</span>
                <Link
                  href={`/machines/detail?id=${data.sourceMachine.id}`}
                  className="underline"
                >
                  {data.sourceMachine.code} (bán nguyên máy)
                </Link>
              </div>
            )}
            {data.readyAt && (
        ...
        <TabsContent value="config">
          <Card>
            <CardContent className="p-0">
              {data.machineSlots && data.machineSlots.length > 0 ? (
                <>
                  {/* Máy bán nguyên chiếc: cấu hình lấy từ kết quả Kiểm tra &
                      định giá của máy gốc (chưa tháo → không có linh kiện kho). */}
                  <div className="border-b bg-muted/50 p-3 text-xs text-muted-foreground">
                    Máy nguyên chiếc — cấu hình theo kết quả Kiểm tra & định giá
                    {data.sourceMachine && (
                      <>
                        {" "}của máy{" "}
                        <Link
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {data.machineSlots.map((s, i) => (
                        <TableRow key={i}>
                          <TableCell>
                            {COMPONENT_CATEGORY_LABEL[s.categoryCode] ?? s.categoryCode}
                          </TableCell>
                          <TableCell>{s.model ?? "-"}</TableCell>
                          <TableCell>{s.serial ?? "-"}</TableCell>
                          <TableCell className="text-right">{s.quantity}</TableCell>
                          <TableCell className="text-right">{formatVnd(s.cost)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                    ...
                    {data.currentComponents.map((c) => (
                      <TableRow key={c.id}>
                        <TableCell className="font-medium">
                          <Link href={`/components/detail?id=${c.id}`} className="underline">{c.code}</Link>
                        </TableCell>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {data.componentHistory.map((h) => (
                    <TableRow key={h.id}>
                      <TableCell className="font-medium">
                        <Link href={`/components/detail?id=${h.componentId}`} className="underline">
                          {h.componentCode}
                        </Link>
                      </TableCell>
