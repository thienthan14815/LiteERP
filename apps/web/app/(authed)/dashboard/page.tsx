...
const QUICK_ACTIONS = [
  { icon: PlusCircle, key: "quick.buy_machine", href: "/purchases/new", color: "text-blue-600" },
  { icon: ShoppingCart, key: "quick.buy_component", href: "/purchases/new", color: "text-emerald-600" },
  { icon: CheckSquare, key: "quick.inspect", href: "/machines", color: "text-amber-600" },
  { icon: Wrench, key: "quick.disassemble", href: "/machines", color: "text-rose-600" },
  { icon: Hammer, key: "quick.assemble", href: "/assemblies/new", color: "text-orange-600" },
  { icon: ShoppingBag, key: "quick.create_sale", href: "/sales/new", color: "text-indigo-600" },
  { icon: PackageOpen, key: "quick.import_component", href: "/components", color: "text-cyan-600" },
];
...
export default function DashboardPage() {
  ...
  const alerts: AlertItem[] = [
    ...
    { icon: ShieldCheck, iconBg: "bg-violet-50", iconColor: "text-violet-500", label: t("alert.warranty_overdue"), count: 5, href: "/warranties", badgeColor: "bg-violet-100 text-violet-700" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        {/* Line chart */}
        <div className="card-soft p-5 lg:col-span-5">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-base font-semibold text-slate-900">{t("chart.revenue_profit")}</h3>
            <select className="rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-600 focus:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-50">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={CHART_DATA} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              ...
              <Line type="monotone" dataKey="revenue" name={t("chart.revenue")} stroke="#3b82f6" strokeWidth={2.5} dot={{ r: 4, fill: "#3b82f6" }} activeDot={{ r: 6 }} />
        ...
        {/* Donut chart */}
        <div className="card-soft p-5 lg:col-span-4">
          ...
          {QUICK_ACTIONS.map((a) => {
            ...
            return (
              <Link
                key={a.key}
                href={a.href}
                className="flex flex-col items-center gap-2 rounded-xl border border-slate-200 bg-white p-4 text-center transition-all hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-50/30 hover:shadow-md"
              >
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((r) => (
              <tr key={r.code} className="hover:bg-slate-50">
                <td className="px-5 py-3 font-medium text-blue-600">{r.code}</td>
