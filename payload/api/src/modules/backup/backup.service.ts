...
export class BackupService {
  ...
  async dumpAndUpload(): Promise<BackupResult> {
    ...
    const tmpDbPath = join(tmpdir(), sqliteFilename);
    try {
  ...
  async restore(
    ...
    // Ghi đè file DB + xoá WAL/SHM (nếu có) rồi mở lại connection sạch.
    try {
    ...
    // Audit "done" ghi qua connection MỚI vào DB vừa restore — cùng hành vi
    // với Prisma trước đây (reconnect sau $disconnect).
    await this.audit.record({
